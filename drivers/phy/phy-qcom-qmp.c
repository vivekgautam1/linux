...
#define PCIE_LANE_OFFSET	0x1000

struct qmp_phy_init_tbl {
	unsigned int reg_offset;
	unsigned int cfg_val;
	/* register part of layout ?
	 * if yes, then reg_offset gives index of the layout
	 */
	int in_layout;
};
...
static struct qmp_phy_init_tbl pciephy_rx_init_tbl[] = {
	...
	QCOM_QMP_PHY_INIT_CFG(QSERDES_RX_SIGDET_LVL, 0x17),
	...
	/* moved above in the sequence, any consequence ?? */
...
struct qmp_phy_init_cfg {
	...
	/*
	 * offset of each lane addr from base addr:
	 * lane addr = lane_no * lane_offset + base
	 **/
	unsigned int lane_offset;
	/* few of the PHYs have a separate PHY_COM_CNTRL block */
 ...
 * @vdda_pll: vdd supply to ref_clk block
...
struct qmp_phy_init_cfg pciephy_init_cfg = {
...
static int qcom_qmp_phy_poweron(struct phy *phy)
{
	...
	dev_err(&phy->dev, "Powering on QMP phy\n");
	...
	if (!qphy->clk_enabled) {
		...
		clk_prepare_enable(qphy->pipe_clk);
...
static int qcom_qmp_phy_com_init(struct qcom_qmp_phy *qphy)
{
	...
	struct qmp_phy_init_cfg *cfg = qphy->cfg;
	int ret;

	if (qphy->inited)
		return 0;

	/* Deassert PHY CSR reset */
	...
	/* Deassert PHY common block reset */
	...
	/* some of the PHYs have separate COM_CONTROL block */
	...
	ret = qcom_qmp_phy_is_ready(qphy, serdes +
					...
					MASK_COM_PCS_READY);
	if (ret) {
		dev_err(qphy->dev, "common block initialization timed-out\n");
		return ret;
	}
	qphy->inited = true;
...
static int qcom_qmp_phy_init(struct phy *phy)
{
	struct qmp_phy_desc *phydesc = phy_get_drvdata(phy);
	struct qcom_qmp_phy *qphy = phydesc->qphy;
	int lane_id = phydesc->index;
	struct qmp_phy_init_cfg *cfg;
	void __iomem *tx, *rx, *pcs;
	int ret;

	dev_err(qphy->dev, "Initializing QMP phy\n");

	/* setup init config for each PHY type */
	switch (qphy->type) {
	case PHY_TYPE_PCIE:
		qphy->cfg = &pciephy_init_cfg;
		break;
	case PHY_TYPE_UFS:
		dev_info(qphy->dev, "**** UFS not supported yet ****\n");
		break;
	case PHY_TYPE_USB3:
		qphy->cfg = &usb3phy_init_cfg;
		break;
	default:
		return -EINVAL;
	}
	...
	/* actual tx, rx, and pcs offsets for individual lanes */
	tx = qphy->tx + lane_id * cfg->lane_offset;
	rx = qphy->rx + lane_id * cfg->lane_offset;
	pcs = qphy->pcs + lane_id * cfg->lane_offset;

	ret = qcom_qmp_phy_com_init(qphy);
	if (ret)
		return ret;

	/* Deassert Lane reset, if any */
	if (phydesc->lane_rst) {
	...
	/* 10 us delay, as given in PCIE HPG only  */
...
static int qcom_qmp_phy_exit(struct phy *phy)
{
	...
	/* PHY reset */
	qphy_setbits(PHY_SW_RESET, qphy->base + USB3_PHY_SW_RESET);

	/* stop SerDes and Phy-Coding-Sublayer */
	qphy_clrbits(PHY_SERDES_START | PHY_PCS_START,
			qphy->base + USB3_PHY_START_CTRL);

	/* Put PHY into POWER DOWN state: active low */
	qphy_clrbits(PHY_SW_PWRDN,
			qphy->base + USB3_PHY_POWER_DOWN_CONTROL);

	/* assert USB3 PHY CSR reset */
	if (qphy->phy_rst) {
		ret = reset_control_assert(qphy->phy_rst);
		if (ret) {
	...
	if (qphy->phycom_rst) {
		ret = reset_control_assert(qphy->phycom_rst);
		if (ret) {
			dev_err(&phy->dev, "common reset deassert failed\n");
			return ret;
		}
	}
#endif
...
static int qcom_qmp_phy_clk_init(struct device *dev)
{
	...
	if (IS_ERR(qphy->pipe_clk)) {
		if (qphy->type == PHY_TYPE_PCIE ||
		    qphy->type == PHY_TYPE_USB3) {
...
static void __iomem *qcom_qmp_phy_res_init(struct platform_device *pdev,
							const char *name)
{
	struct resource *res;
					...
					struct of_phandle_args *args)
{
	...
	if (WARN_ON(args->args[1] >= qphy->nlanes))
		return ERR_PTR(-ENODEV);

	for (i = 0; i < qphy->nlanes; i++) {
		if (qphy->phys[i]->index == args->args[1])
			break;
	}
...
static int qcom_qmp_phy_probe(struct platform_device *pdev)
{
	...
	/* io-mem resources */
	qphy->serdes = qcom_qmp_phy_res_init(pdev, "serdes");
	if (IS_ERR(qphy->serdes))
		return PTR_ERR(qphy->serdes);
	qphy->tx = qcom_qmp_phy_res_init(pdev, "tx");
	if (IS_ERR(qphy->tx))
		return PTR_ERR(qphy->tx);
	qphy->rx = qcom_qmp_phy_res_init(pdev, "rx");
	if (IS_ERR(qphy->rx))
		return PTR_ERR(qphy->rx);
	qphy->pcs = qcom_qmp_phy_res_init(pdev, "pcs");
	if (IS_ERR(qphy->pcs))
	...
	qphy->phy_rst = devm_reset_control_get_optional(dev, "phy");
	if (IS_ERR(qphy->phy_rst)) {
		dev_err(dev, "failed to get phy core reset\n");
		return PTR_ERR(qphy->phy_rst);
	}

	qphy->phycom_rst = devm_reset_control_get_optional(dev, "common");
	if (IS_ERR(qphy->phycom_rst)) {
		dev_err(dev, "failed to get phy common reset\n");
		return PTR_ERR(qphy->phycom_rst);
	}

	ret = of_property_read_u32(dev->of_node, "nlanes", &qphy->nlanes);
	if (ret)
		return ret;

	qphy->phys = devm_kcalloc(dev, qphy->nlanes, sizeof(*qphy->phys),
				  GFP_KERNEL);
	if (!qphy->phys)
		return -ENOMEM;

	for (id = 0; id < qphy->nlanes; id++) {
		...
		phy_desc->lane_rst = devm_reset_control_get_optional(dev,
								prop_name);
...
static const struct of_device_id qcom_qmp_phy_id_table[] = {
	{
		.compatible = "qcom,qmp-14nm-phy",
	},
	{ },
};
MODULE_DEVICE_TABLE(of, qcom_qmp_phy_id_table);

static struct platform_driver qcom_qmp_phy_driver = {
	.probe		= qcom_qmp_phy_probe,
	.driver = {
		.name	= "qcom_qmp_phy",
		.of_match_table = of_match_ptr(qcom_qmp_phy_id_table),
	},
};
