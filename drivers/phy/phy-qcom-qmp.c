...
static struct qmp_phy_init_tbl pciephy_rx_init_tbl[] = {
	...
	/* XXX: moved above in the sequence, any consequence ?? */
 ...
 * @phycfg_rst: Pointer to phy ahb cfg reset control
 *
...
struct qcom_qmp_phy {
	...
	int nlanes;
	/*
	 * offset of each lane addr from base addr (tx/rx/pcs):
...
static
void __iomem *qcom_qmp_phy_ioremap_res(struct device *dev,
					struct resource *res,
					const char *name, int lane_id)
{
	struct qcom_qmp_phy *qphy = dev_get_drvdata(dev);
	...
	start = res->start + lane_id * qphy->lane_offset;
	size = resource_size(res);

	if (!devm_request_mem_region(dev, start, size, name)) {
...
static int qcom_qmp_phy_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	struct resource *res_tx, *res_rx, *res_pcs;
	int ret = 0;
	int id;
	...
	if (!res)
		return -ENODEV;
	qphy->serdes = devm_ioremap_resource(dev, res);
	if (IS_ERR(qphy->serdes))
		return PTR_ERR(qphy->serdes);

	/* other resources, to get ioremapped addresses based on lane-id */
	res_tx = platform_get_resource_byname(pdev, IORESOURCE_MEM, "tx");
	if (!res_tx)
		return -ENODEV;
	res_rx = platform_get_resource_byname(pdev, IORESOURCE_MEM, "rx");
	if (!res_rx)
		return -ENODEV;
	res_pcs = platform_get_resource_byname(pdev, IORESOURCE_MEM, "pcs");
	if (!res_pcs)
		return -ENODEV;
	...
	qphy->phycfg_rst = devm_reset_control_get_optional(dev, "cfg");
	...
	if (qphy->nlanes > 1) {
		ret = of_property_read_u32(dev->of_node, "lane-offset",
							&qphy->lane_offset);
		if (ret)
			return ret;
	}
	...
	for (id = 0; id < qphy->nlanes; id++) {
		...
		phy_desc->tx = qcom_qmp_phy_ioremap_res(dev, res_tx,
								"tx", id);
		if (IS_ERR(phy_desc->tx))
			return PTR_ERR(phy_desc->tx);

		phy_desc->rx = qcom_qmp_phy_ioremap_res(dev, res_rx,
								"rx", id);
		if (IS_ERR(phy_desc->rx))
			return PTR_ERR(phy_desc->rx);

		phy_desc->pcs = qcom_qmp_phy_ioremap_res(dev, res_pcs,
								"pcs", id);
		if (IS_ERR(phy_desc->pcs))
			return PTR_ERR(phy_desc->pcs);
