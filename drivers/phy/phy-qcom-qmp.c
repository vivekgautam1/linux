...
static int qcom_qmp_phy_probe(struct platform_device *pdev)
{
	...
	qphy->nlanes = of_get_child_count(dev->of_node);
	if (qphy->nlanes == 0)
		return -ENODEV;
	...
	for_each_available_child_of_node(dev->of_node, child) {
		...
		if (of_property_read_u32(child, "tx-offset", &reg_offset)) {
			dev_err(dev, "missing tx-offset property in node %s\n",
				child->name);
			ret = -EINVAL;
			goto put_child;
		}
		phy_desc->tx = base + reg_offset;

		if (of_property_read_u32(child, "rx-offset", &reg_offset)) {
			dev_err(dev, "missing rx-offset property in node %s\n",
				child->name);
			ret = -EINVAL;
			goto put_child;
		}
		phy_desc->rx = base + reg_offset;

		if (of_property_read_u32(child, "pcs-offset", &reg_offset)) {
			dev_err(dev, "missing pcs-offset property in node %s\n",
				child->name);
			ret = -EINVAL;
			goto put_child;
		}
		phy_desc->pcs = base + reg_offset;
		...
		phy_desc->lane_rst = of_reset_control_get(child, prop_name);
